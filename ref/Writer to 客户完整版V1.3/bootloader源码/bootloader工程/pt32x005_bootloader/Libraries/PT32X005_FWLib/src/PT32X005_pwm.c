...
void PWM_OutputInit(CMSDK_PWM_TypeDef* PWMx, PWM_OutputInitTypeDef* OutInit)
{
	...
	if(ch == PWM_CH_1)	PWMx->MR1 = OutInit->CompareValue;
	if(ch == PWM_CH_2)	PWMx->MR2 = OutInit->CompareValue;
	if(ch == PWM_CH_3)	PWMx->MR3 = OutInit->CompareValue;
	if(ch == PWM_CH_4)	PWMx->MR4 = OutInit->CompareValue;
	/* Set the CMR Reg */
	wTmpReg = PWMx->CMR;
...
/**
  ...
  * @param  PWMx: where x can be 0, 1 to select the PWM peripheral.
  * @note
  * @param  CapInit: pointer to a PWM_CaptureInitTypeDef
  *         structure that contains the configuration information for
  *         the specified PWM peripheral.
  * @retval None
  */
...
void PWM_CaptureInit(CMSDK_PWM_TypeDef* PWMx, PWM_CaptureInitTypeDef* CapInit)
{
	...
	/* Check the parameters */
	assert_param(IS_PWM_ALL_PERIPH(PWMx));
	assert_param(IS_PWM_CHCHANNEL(CapInit->Channel));
	assert_param(IS_PWM_CM(CapInit->CaptureMode));
	assert_param(IS_PWM_CMINT(CapInit->Capture_Interrupt));

	ch = CapInit->Channel;
	PWMx->CMR &= ~(0x03 << ch * 4);	//禁止CHx 输出模式
	/* Set the CCR Reg */
	wTmpReg = PWMx->CCR;
	wTmpReg &= ~(0x0F << ch * 4);
	wTmpReg |=  (((CapInit->CaptureMode) << ch * 4) | ((CapInit->Capture_Interrupt) << ch));
	PWMx->CCR = wTmpReg;
...
void PWM_BKI_LevelConfig(CMSDK_PWM_TypeDef* PWMx, uint32_t BKI_Level)
{
	...
	/* Check the parameters */
	assert_param(IS_PWM_ALL_PERIPH(PWMx));
	assert_param(IS_PWM_BIK_LEVEL(BKI_Level));
	wTmpReg = PWMx->CMR;
	wTmpReg &= ~PWM_CMR_BKP;
	wTmpReg |= BKI_Level;
	PWMx->CMR = wTmpReg;
