...
//	I2Cx->CCR = I2C_CCR_CR;
	...
	I2Cx->CCR |= I2C_CSR_DIV;
	I2Cx->CSR	|= (((I2C_InitStruct->I2C_SCLK_DIV/4)-1) << 16);        
}
...
void I2C_TransferHandling(I2C_InstTypeDef* pInst, uint8_t StatValue)
{
	switch ( StatValue )
	{
		...
		case 0xB0:
			...
			pInst->pInstance->DR = pInst->pWriteData[pInst->WrIndex++];/* write the same data back to master */
			pInst->pInstance->CCR = I2C_CCR_IS;
			break;
...
uint32_t I2C_StartHandling(I2C_InstTypeDef* pInst)
{
	...
	pInst->pInstance->CSR = I2C_CSR_EN | I2C_CSR_START|I2C_CSR_IS;
...
uint32_t i2cMaster_TransmitStart(I2C_InstTypeDef* pInst,uint8_t opByte,uint8_t *pData,uint32_t datLen)
{
	...
	pInst->RdIndex = 0;	pInst->WrIndex = 0;
