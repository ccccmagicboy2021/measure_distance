...
//	System_GetClocksFreq(&System_ClocksStatus);
	pclk = 8000000;//System_ClocksStatus.PCLK_Frequency;
...
/**
  ...
  * @param  USARTx: where x can be 0 or 1  to select the UART peripheral.
  * @param  NewState: new state of the Smart Card mode.
  *          This parameter can be: ENABLE or DISABLE.
  * @retval None
  */
void USART_SmartCardCmd(CMSDK_UART_TypeDef* UARTx, FunctionalState NewState)
{
	/* Check the parameters */
	assert_param(IS_UART_ALL_PERIPH(UARTx));
	assert_param(IS_FUNCTIONAL_STATE(NewState));
