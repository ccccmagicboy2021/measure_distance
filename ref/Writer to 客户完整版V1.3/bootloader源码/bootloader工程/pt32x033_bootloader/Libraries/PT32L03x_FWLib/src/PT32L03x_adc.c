...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define INICFR_CLEAR_MASK           ((uint32_t)0x000340BF)
...
void ADC_Init(CMSDK_ADC_TypeDef* ADCx, ADC_InitTypeDef* ADC_InitStruct)
{
	...
	assert_param(ADC_Vref_P_Set(ADC_InitStruct->ADC_Vref_P));
	assert_param(ADC_Vref_N_Set(ADC_InitStruct->ADC_Vref_N));
	...
	tmpreg  |= (ADC_InitStruct->ADC_ContinuousConvMode) |
			...
			(ADC_InitStruct->ADC_ExternalTrigConv)|
			(ADC_InitStruct->ADC_Vref_P)|
			(ADC_InitStruct->ADC_SynClkDiv) |
			(ADC_InitStruct->ADC_Vref_N);
	/* Write to ADCx CFGR */
	ADCx->CR = tmpreg;
}
...
void ADC_StructInit(ADC_InitTypeDef* ADC_InitStruct)
{
	...
	ADC_InitStruct->ADC_SynClkDiv=ADC_ClockMode_SynClkDiv2;
	...
	ADC_InitStruct->ADC_Vref_P= ADC_Vref_P_BG2V;//ADC_Vref_P_VDDA;//ADC_Vref_P_BG2V;
	ADC_InitStruct->ADC_Vref_N=ADC_Vref_N_VSSA;
}
...
/**
  ...
  *            @arg ADC_Channel_Vout_2V: ADC_Channel_Vout_2V 
  *            @arg ADC_Channel_Vrefh_3v: ADC_Channel_Vrefh_3v 
  *            @arg ADC_Channel_Vrefl_3v: ADC_Channel_Vrefl_3v
  *            @arg ADC_Channel_ANA_3V: ADC_Channel_ANA_3V
  *            @arg ADC_Channel_VBG_3V: ADC_Channel_VBG_3V 	
  *            @arg ADC_Channel_LP5V: ADC_Channel_LP5V 	
  *            @arg ADC_Channel_VLCD_3V: ADC_Channel_VLCD_3V 
  * @retval None
  */
