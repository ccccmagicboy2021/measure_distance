...
uint32_t SerialKeyPressed(uint8_t *key)
{
    ...
    if ( (((UART2->SR)&0x01)) != RESET)//接收缓冲队列非空。
    {
        *key = (uint8_t) UART2->BR;//读到的数据，并返回。
        return 1;
    }
    else
    {
...
//																}
																...
																{
																	 FLASHStatus=FLASH_ErasePage(FlashDestination + (FLASH_PAGE_SIZE * EraseCounter));
													 ...
													 for (j = 0; (j < packet_length) && (FlashDestination <  ApplicationAddress + size); j += 8)	
														{
															...
															FLASHStatus = FLASH_ProgramWord(FlashDestination, *(uint32_t*)RamSource,*(uint32_t*)(RamSource+4));
