...
void System_GetClocksFreq(System_ClocksTypeDef * System_Clocks)
{
  ...
  {
    System_Clocks->SYSCLK_Frequency = IOSC_RC32M_VALUE /(SystemCoreClockDiv+1);	
  }
  ...
  else if (SystemCoreClockSrc==2) /*  */
  {
		 if(DoubleClockSrc == 0) /*  */
		 {			 
     System_Clocks->SYSCLK_Frequency = (IOSC_RC32M_VALUE*2) /(SystemCoreClockDiv+1);
		 }		 
     ...
     System_Clocks->SYSCLK_Frequency = (EOSC_CRYSTAL_VALUE*2) /(SystemCoreClockDiv+1);
		 ...
		 {			 
     System_Clocks->SYSCLK_Frequency = (IOSC_32K_VALUE*2) /(SystemCoreClockDiv+1);	 
		 }			 
  }		
  else /*  */
  {
    System_Clocks->SYSCLK_Frequency = IOSC_32K_VALUE /(SystemCoreClockDiv+1);		
  }
...
void IAP_Config(FunctionalState NewState)
{
  assert_param(IS_FUNCTIONAL_STATE(NewState));
	if(NewState != DISABLE)
	{
	 CMSDK_SYSCON->REMAP |= SYSCON_REMAP_IAP_EN;
	}
	else
	{
	 CMSDK_SYSCON->REMAP &= ~SYSCON_REMAP_IAP_EN;
...
{
  uint32_t tmp;
  tmp = (CMSDK_SYSCON->FLASHPROTECT & (0x00000001));
  if (tmp != RESET)
  {
    return SET;
  }
...
{
  CMSDK_SYSCON->REMAP &= ~SYSCON_REMAP_BOOT;
}
...
{
  return ( CMSDK_SYSCON->CUSTOMER_ID );
}


uint32_t System_GetAgentID(void)
{
  return ( CMSDK_SYSCON->AGENTID );
}
