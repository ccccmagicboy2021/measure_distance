...
void TMR0_Handler(void)
{
   static int b=0;
  /*产生一次上升沿捕获中断*/
	if(PWM_GetFlagStatus(PWM0,PWM_FLAG_CAP1_RE))
	{
		b++;
		if(b%2==1){
	  /*TC复位*/
    ...
    if(b%2==0){
		...
		/*计算两个上升沿之间总共的COUNT数*/
		keypress_counter= PWM0->TC+(occurred*60000);
		occurred =0;
		printf("两个上升沿之间时间是 : %d us\r\n",keypress_counter);

	 }
 
	 	 /*清除TIMER1的输入上升沿捕获中断标识*/
		PWM_ClearFlag(PWM0, PWM_FLAG_CAP1_RE);
	}
}
