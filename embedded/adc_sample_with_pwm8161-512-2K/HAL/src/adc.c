...
TESK_ID_TYPEDEF TaskVal = TESK1;		//任务ID
Task_TYPEDEF TaskState = Task_STATE1;	//任务状态
TaskCallBack pTaskCBS;       			//定义函数指针原型
...
{
	int ret;
	usart_polling_send_adc_512((uint8_t *)(ring_buffer.buf + ring_buffer.rd * ring_buffer.elem_size));
	ret = ring_buffer_get(&ring_buffer);
