  ...
  { 
    CMSDK_FLASH->AR  = Page_Address;
      ...
      status = FLASH_WaitForLastOperation(FLASH_ER_PRG_TIMEOUT);  			
		...
		else if((((Page_Address) <= NVR_END_ADDR)&&((Page_Address) >= NVR_START_ADDR)))
		{
...
FLASH_Status FLASH_ProgramWord(uint32_t Address, uint32_t Data0,uint32_t Data1)
{
  ...
  CMSDK_FLASH->ACR = (FLASH_FKEY1|tmpreg);
      ...
      if((status ==FLASH_COMPLETE)&& ((Address) >= NVR_START_ADDR) && ((Address) <= NVR_END_ADDR)&&((Address&0x07)==0))
			{			
        tmpreg = CMSDK_FLASH->ACR;
...
//		{		
//      /* Data is programming into the user information area */
//      if( ((Address) >= NVR_START_ADDR) && ((Address) <= NVR_END_ADDR)&&((Address&0x07)==0))
//			{				
//		    CMSDK_FLASH->ACR = (FLASH_FKEY2|tmpreg);	
//			  status = FLASH_WaitForLastOperation(FLASH_ER_PRG_TIMEOUT); 
//			}	
